...
> 制定日期: 2026-07-13 | 基线版本: v1.4.5 | 当前平台: Windows x64

---

## 🎯 规划总纲

本规划的核心思路是 **"今日修路，明日通车"**——在当前 Windows 开发过程中，逐步完成架构重构，
使未来的 Linux（含 ARM）迁移从"伤筋动骨的大工程"降级为"几天就能完成的配置工作"。

| 目标 | 含义 |
|------|------|
| **方便迁移 Linux** | 在 Windows 开发中建立平台抽象，消除一切未来 Linux 迁移的代码障碍 |
| **修正硬编码** | 将所有 `.exe`、Windows 专有路径、架构假设提取为可配置的平台抽象 |
| **完善 Windows 识别** | 增强当前 Windows 平台的编码器/工具自动发现能力 |
| **优化运行效率** | 管道模式全面化、内存管理、启动性能 |

> ⚠️ **当前阶段不编译 Linux 二进制，不在 Linux 上测试。** 所有改动均在 Windows 上开发和验证，
> 目的是让代码"准备好"跨平台，而非"已经"跨平台。

---

## 目录

- [第一阶段 P0：平台抽象层 + 硬编码消除（2 周）](#第一阶段-p0平台抽象层--硬编码消除2-周)
- [第二阶段 P1：架构安全化 + 可移植性加固（1-2 周）](#第二阶段-p1架构安全化--可移植性加固1-2-周)
- [第三阶段 P2：Windows 平台识别完善（1 周）](#第三阶段-p2windows-平台识别完善1-周)
- [第四阶段 P3：性能优化（2-3 周）](#第四阶段-p3性能优化2-3-周)
- [附录 A：完整文件改动清单](#附录-a完整文件改动清单)
